...
void
GameObject::death(Level & level, std::shared_ptr<GameObject> obj)
{
...
GameObject::render(SDL_Renderer * renderer)
{
  ...
  if (mRenderComponent) {
    ...
    if (animUpdate == 0) {
      mRenderComponent->step();
    }
    animUpdate = (animUpdate - 1) % 5;
  }
...
void
GameObject::render(SDL_Renderer * renderer, std::shared_ptr<GameObject> cam) 
{
  if (mRenderComponent) {
    mRenderComponent->render(renderer, cam);
    if (animUpdate == 0) {
      mRenderComponent->step();
    }
    animUpdate = (animUpdate - 1) % 5;
