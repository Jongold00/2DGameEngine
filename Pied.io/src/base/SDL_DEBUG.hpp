// replacement functions for debugging some parts of SDL use for resource manager lab
...
/**
 * @file SDL_DEBUG.hpp
 * @brief debugging for SDL textures
 */
...
inline SDL_Texture * __DEBUG__SDL_CreateTextureFromSurface(SDL_Renderer * renderer, SDL_Surface * surface)
{
  SDL_Texture * texture = SDL_CreateTextureFromSurface(renderer, surface);
  if (texture) {
    __DEBUG__textures().insert(texture);
  }
  std::cout << "SDL_DEBUG: Creating texture; " << __DEBUG__textures().size() << " textures loaded." << std::endl;
  return texture;
}
...
inline void __DEBUG__SDL_DestroyTexture(SDL_Texture* texture)
{
  ...
  std::cout << "SDL_DEBUG: Destroying texture; " << __DEBUG__textures().size() << " textures loaded." << std::endl;
}
...
inline SDL_Surface * __DEBUG__SDL_LoadBMP_RW(SDL_RWops * src, int freesrc)
{
  ...
  std::cout << "SDL_DEBUG: Loading surface; " << __DEBUG__surfaces().size() << " surfaces loaded." << std::endl;
  return surface;
}
...
inline void __DEBUG__SDL_DestroySurface(SDL_Surface* surface)
{
  ...
  std::cout << "SDL_DEBUG: Freeing surface; " << __DEBUG__surfaces().size() << " surfaces loaded." << std::endl;
}
...
{
  std::cout << "SDL_DEBUG: Quitting SDL; " << __DEBUG__textures().size() << " textures and " << __DEBUG__surfaces().size() << " surfaces loaded." << std::endl;
  SDL_Quit();
}
