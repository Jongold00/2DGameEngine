...
public:
  ...
  virtual void render(SDL_Renderer * renderer); //!< Render the object.
  virtual void render(SDL_Renderer * renderer, std::shared_ptr<GameObject> cam); //!< Render the object on the camera's offset
  virtual void death(Level & level, std::shared_ptr<GameObject> obj); //!<the gameobject's reaction to being destroyed
