...
{
public:
  PlatformInputComponent(GameObject &gameObject, float speed, float jump, float gravity) : GenericComponent(gameObject),
                                                                                           mSpeed(speed),
                                                                                           mJump(jump),
                                                                                           mGravity(gravity)
  {
...
Player::Player(int x, int y, int w, int h) : GameObject(x, y, w * 0.9, h * 0.9, 0)
{
  ...
  this->run_tex = ResourceManager::getInstance().getTexture("platformer/Run (32x32).png");
  std::pair<SDL_Texture *, int> run = make_pair(run_tex, 12);

  this->idle_tex = ResourceManager::getInstance().getTexture("platformer/idle.png");
  std::pair<SDL_Texture *, int> idle = make_pair(idle_tex, 11);

  this->jump_tex = ResourceManager::getInstance().getTexture("platformer/Jump (32x32).png");
  std::pair<SDL_Texture *, int> jump = make_pair(jump_tex, 1);

  this->fall_tex = ResourceManager::getInstance().getTexture("platformer/Fall (32x32).png");
  ...
  addGenericCompenent(std::make_shared<PlatformInputComponent>(*this, 5.0f, 12.0f, 0.5f));

  addGenericCompenent(std::make_shared<RemoveOnCollideComponent>(*this, Collectable::COLLECTABLE_TAG));
  addGenericCompenent(std::make_shared<RemoveOnCollideComponent>(*this, Goal::GOAL_TAG));
