 ...
 * @return Level* 
 */
std::shared_ptr<Level> LevelLoader::loadLevelFromPath(const std::string & filename, std::shared_ptr<Level> level)
{
  ...
  LevelLoader::textureWidth = level->w() / c;
  LevelLoader::textureHeight = level->h() / r;
  ...
  while(getline(infile, currLine)) 
  {
    ...
    for (auto it = currLine.begin(); it < currLine.end(); it++)
    {
      ...
      {
        level->addObject(std::make_shared<PlatformerBlock>(xPad, yPad, textureWidth, textureHeight));
...
void LevelLoader::writeToNewFile(Level* level)
{
  ...
  ofstream tempFile;
  tempFile.open("./res/platformer/levels/" + newFileName + ".txt", ios::out);
